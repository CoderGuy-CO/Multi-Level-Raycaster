namespace SpriteKind {
    export const collectible = SpriteKind.create()
    export const collectible1 = SpriteKind.create()
    export const collectible2 = SpriteKind.create()
    export const collectible3 = SpriteKind.create()
}
/**
 * test for RelativeToCamera
 * 
 * game.onUpdateInterval(3000, ()=>{
 * 
 * cake.setFlag(SpriteFlag.RelativeToCamera, !(cake.flags&SpriteFlag.RelativeToCamera))
 * 
 * })
 */
/**
 * controller.B.onEvent(ControllerButtonEvent.Released, () => {
 */
/**
 * })
 */
/**
 * rcRender.fov=fov
 */
/**
 * }
 */
sprites.onOverlap(SpriteKind.Player, SpriteKind.collectible, function (sprite, otherSprite) {
    score += 1
    mySprite.destroy()
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.collectible1, function (sprite, otherSprite) {
    score += 1
    mySprite2.destroy()
})
/**
 * pause(20)
 */
controller.anyButton.onEvent(ControllerButtonEvent.Pressed, function () {
    if (controller.B.isPressed()) {
        if (controller.A.isPressed()) {
            Render.moveWithController(0, 0)
            isAdjusting = true
        } else {
            rcRender.jumpWithHeightAndDuration(rcRender.sprSelf, tilemapScale, 500)
        }
    }
})
/**
 * for (let fov = Render.defaultFov; fov > Render.defaultFov - .6; fov -= .06) {
 */
function createSprite (x: number, y: number, vx: number, vy: number, textures: Image[][], kind: number) {
    spr = sprites.create(textures[0][0], kind)
    const tilemapScale2 = 1 << game.currentScene().tileMap.scale
spr.setPosition(x * tilemapScale2, y * tilemapScale2)
    spr.setVelocity(vx, vy)
    spr.setBounceOnWall(true)
    spr.setScale(0.5, ScaleAnchor.Middle)
    // setCharacterAnimationForSprite(spr, textures)
    Render.setSpriteAnimations(spr, Render.createAnimations(150, textures[0], textures[1], textures[2], textures[3]))
    // if (kind == SpriteKind.Enemy)
    // tiles.placeOnRandomTile(spr, trans16)
    spr.sayText("" + spr.id + " test", 444444)
    // spr.startEffect(effects.fountain,9999)
    return spr
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.collectible3, function (sprite, otherSprite) {
    score += 1
    mySprite4.destroy()
})
/**
 * rcRender.setZOffset(sprSkelly, 4)
 * 
 * rcRender.setZOffset(sprPlane, 16)
 * 
 * rcRender.setZOffset(cake, 4)
 * 
 * rcRender.setZOffset(fish, 8)
 * 
 * for(let i=0;i<10;i++){
 * 
 * let spr=createSprite(4, 7, Math.randomRange(5,10), Math.randomRange(3,10), texturesCoin, SpriteKind.Food)
 * 
 * tiles.placeOnRandomTile(spr, trans16)
 * 
 * rcRender.setZOffset(spr,.25)
 * 
 * }
 */
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    music.pewPew.play()
    s = sprites.createProjectileFromSprite(sprites.projectile.bubble1, rcRender.sprSelf, rcRender.dirX * 55, rcRender.dirY * 55)
    s.setScale(0.25, ScaleAnchor.Middle)
    // todo, use VP height
    rcRender.setZOffset(s, rcRender.getMotionZPosition(rcRender.sprSelf) + 2)
})
sprites.onOverlap(SpriteKind.Enemy, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (!(rcRender.isOverlapZ(sprite, otherSprite))) {
        return
    }
    otherSprite.setVelocity(otherSprite.x - sprite.x, otherSprite.y - sprite.y)
    sprite.setVelocity(0 - (otherSprite.x - sprite.x), 0 - (otherSprite.y - sprite.y))
})
sprites.onOverlap(SpriteKind.Enemy, SpriteKind.Projectile, function (sprite, otherSprite) {
    if (!(rcRender.isOverlapZ(sprite, otherSprite))) {
        return
    }
    sprite.setVelocity(0, 0)
    sprite.sayText("No!", 2000)
    rcRender.move(sprite, 60, -160)
rcRender.setZOffset(sprite, 0)
music.baDing.play()
    score += 1
    otherSprite.destroy()
    // Render.setSpriteAnimations(sprite, new Render.Animations(120, texturesCoin))
    // sprite.setImage(sprites.builtin.coin0)
    sprite.setScale(0.5, ScaleAnchor.Middle)
    sprite.startEffect(effects.fire, 5000)
    sprite.destroy()
})
info.onCountdownEnd(function () {
    game.over(false)
})
// music.pewPew.play()
// otherSprite.destroy()
sprites.onOverlap(SpriteKind.Enemy, SpriteKind.Food, function (sprite, otherSprite) {
	
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.collectible2, function (sprite, otherSprite) {
    score += 1
    mySprite3.destroy()
})
controller.A.onEvent(ControllerButtonEvent.Released, function () {
    isAdjusting = false
    Render.moveWithController(2, 2)
})
/**
 * controller.B.onEvent(ControllerButtonEvent.Pressed, () => {
 */
/**
 * 0<= dir <1, then may be added by 2 for avoid negative
 * 
 * rcRender.registerOnSpriteDirectionUpdate((spr, dir) => {
 * 
 * // character.setCharacterState(spr, character.rule(characterAniDirs[Math.floor(dir * 4 + .5) % 4]))
 * 
 * })
 */
controller.menu.onEvent(ControllerButtonEvent.Pressed, function () {
    Render.toggleViewMode()
})
controller.B.onEvent(ControllerButtonEvent.Released, function () {
    isAdjusting = false
    Render.moveWithController(2, 2)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Food, function (sprite, otherSprite) {
    if (!(rcRender.isOverlapZ(sprite, otherSprite))) {
        return
    }
    music.baDing.play()
    info.changeLifeBy(1)
    otherSprite.destroy()
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (!(rcRender.isOverlapZ(sprite, otherSprite))) {
        return
    }
})
/**
 * }
 */
/**
 * })
 */
/**
 * character.loopFrames(spr, textures[Math.floor(i * textures.length / characterAniDirs.length)], 150, character.rule(dir))
 */
/**
 * characterAniDirs.forEach((dir, i) => {
 */
/**
 * function setCharacterAnimationForSprite(spr: Sprite, textures: Image[][]) {
 */
/**
 * const characterAniDirs = [Predicate.MovingLeft, Predicate.MovingDown, Predicate.MovingRight, Predicate.MovingUp]
 */
/**
 * })
 */
/**
 * }
 */
/**
 * pause(20)
 */
/**
 * rcRender.fov=fov
 */
/**
 * for (let fov = Render.defaultFov - .6; fov <= Render.defaultFov; fov += .06) {
 */
let s: Sprite = null
let spr: Sprite = null
let isAdjusting = false
let mySprite4: Sprite = null
let mySprite3: Sprite = null
let mySprite2: Sprite = null
let mySprite: Sprite = null
let count = 0
game.stats = true
const rcRender = Render.raycastingRender
Render.moveWithController(2, 2, 1)
let trans16 = image.create(16, 16)
scene.setBackgroundImage(img`
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    ................................................................................................................................................................
    
    `)
let map = tiles.createTilemap(hex``, img`
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . 2 2 2 . . 2 2 2 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . 2 . . . . . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 2 2 2 . . 2 2 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
`, [trans16, sprites.castle.tileGrass2, sprites.builtin.forestTiles0], TileScale.Sixteen);
// tiles.setCurrentTilemap(map)
let tm = tilemap`level`
// tm.scale =2// TileScale.Eight
tiles.setCurrentTilemap(tm)
const tilemapScale = 1 << game.currentScene().tileMap.scale
rcRender.sprSelf.setPosition(8 * tilemapScale, 8 * tilemapScale)
createSprite(8, 7, 6, 10, texturesDuck, SpriteKind.Enemy)
createSprite(6, 7, 6, 10, texturesDonut, SpriteKind.Enemy)
createSprite(5, 8, 6, 10, texturesDog, SpriteKind.Enemy)
let sprPriness2 = createSprite(11, 8, 6, 10, texturesPrincess2, SpriteKind.Enemy)
let sprHero = createSprite(10, 8, 6, 10, texturesHero, SpriteKind.Enemy)
let sprSkelly = createSprite(9, 9, 6, 10, texturesSkelly, SpriteKind.Enemy)
let sprPriness = createSprite(10, 7, 6, 10, texturesPrincess, SpriteKind.Enemy)
let sprPlane = createSprite(9, 7, 6, 10, texturesPlane, SpriteKind.Enemy)
let cake = createSprite(2, 2, 4, 2, texturesBigCake, SpriteKind.Enemy)
let fish = createSprite(7, 9, 0, 0, texturesFish, SpriteKind.Enemy)
controller.B.repeatDelay = 0
rcRender.wallZScale = 2
let zOffset = 3
rcRender.setZOffset(rcRender.sprSelf, zOffset, 0)
let fov = Render.defaultFov
let score = 0
info.startCountdown(120)
mySprite = sprites.create(img`
    . . b b b b . . 
    . b 5 5 5 5 b . 
    b 5 d 3 3 d 5 b 
    b 5 3 5 5 1 5 b 
    c 5 3 5 5 1 d c 
    c d d 1 1 d d c 
    . f d d d d f . 
    . . f f f f . . 
    `, SpriteKind.collectible)
mySprite2 = sprites.create(img`
    . . b b b b . . 
    . b 5 5 5 5 b . 
    b 5 d 3 3 d 5 b 
    b 5 3 5 5 1 5 b 
    c 5 3 5 5 1 d c 
    c d d 1 1 d d c 
    . f d d d d f . 
    . . f f f f . . 
    `, SpriteKind.collectible1)
mySprite3 = sprites.create(img`
    . . b b b b . . 
    . b 5 5 5 5 b . 
    b 5 d 3 3 d 5 b 
    b 5 3 5 5 1 5 b 
    c 5 3 5 5 1 d c 
    c d d 1 1 d d c 
    . f d d d d f . 
    . . f f f f . . 
    `, SpriteKind.collectible2)
mySprite4 = sprites.create(img`
    . . b b b b . . 
    . b 5 5 5 5 b . 
    b 5 d 3 3 d 5 b 
    b 5 3 5 5 1 5 b 
    c 5 3 5 5 1 d c 
    c d d 1 1 d d c 
    . f d d d d f . 
    . . f f f f . . 
    `, SpriteKind.collectible3)
tiles.placeOnRandomTile(mySprite, sprites.dungeon.collectibleInsignia)
tiles.placeOnRandomTile(mySprite2, sprites.dungeon.chestOpen)
tiles.placeOnRandomTile(mySprite3, sprites.dungeon.chestClosed)
tiles.placeOnRandomTile(mySprite4, sprites.castle.rock1)
tileUtil.replaceAllTiles(sprites.dungeon.collectibleInsignia, assets.tile`transparency16`)
tileUtil.replaceAllTiles(sprites.dungeon.chestOpen, assets.tile`transparency16`)
tileUtil.replaceAllTiles(sprites.dungeon.chestClosed, assets.tile`transparency16`)
tileUtil.replaceAllTiles(sprites.castle.rock1, assets.tile`transparency16`)
game.onUpdate(function () {
    if (isAdjusting) {
        // zOffset -= controller.dy(10)
        // rcRender.setZOffset(rcRender.sprSelf, zOffset, 0)
        fov += 0 - controller.dy(1)
        Render.setAttribute(Render.attribute.fov, fov)
        info.setScore(zOffset * 100)
    }
})
game.onUpdateInterval(100, function () {
    if (score == 14) {
        count = 10
        game.stats = true
const rcRender = Render.raycastingRender
Render.moveWithController(2, 2, 1)
        trans16 = image.create(16, 16)
        scene.setBackgroundImage(img`
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            
            `)
        let map = tiles.createTilemap(hex``, img`
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . 2 2 2 . . 2 2 2 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . 2 . . . . . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 2 2 2 . . 2 2 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
`, [trans16, sprites.castle.tileGrass2, sprites.builtin.forestTiles0], TileScale.Sixteen);
// tiles.setCurrentTilemap(map)
        tm = tilemap`level2`
        // tm.scale =2// TileScale.Eight
        tiles.setCurrentTilemap(tm)
        const tilemapScale = 1 << game.currentScene().tileMap.scale
rcRender.sprSelf.setPosition(8 * tilemapScale, 8 * tilemapScale)
        createSprite(8, 7, 6, 10, texturesDuck, SpriteKind.Enemy)
        createSprite(6, 7, 6, 10, texturesDonut, SpriteKind.Enemy)
        createSprite(5, 8, 6, 10, texturesDog, SpriteKind.Enemy)
        sprPriness2 = createSprite(11, 8, 6, 10, texturesPrincess2, SpriteKind.Enemy)
        sprHero = createSprite(10, 8, 6, 10, texturesHero, SpriteKind.Enemy)
        sprSkelly = createSprite(9, 9, 6, 10, texturesSkelly, SpriteKind.Enemy)
        sprPriness = createSprite(10, 7, 6, 10, texturesPrincess, SpriteKind.Enemy)
        sprPlane = createSprite(9, 7, 6, 10, texturesPlane, SpriteKind.Enemy)
        cake = createSprite(2, 2, 4, 2, texturesBigCake, SpriteKind.Enemy)
        fish = createSprite(7, 9, 0, 0, texturesFish, SpriteKind.Enemy)
        controller.B.repeatDelay = 0
rcRender.wallZScale = 2
zOffset = 3
        rcRender.setZOffset(rcRender.sprSelf, zOffset, 0)
let fov = Render.defaultFov
score = 15
        info.startCountdown(120)
        mySprite = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible)
        mySprite2 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible1)
        mySprite3 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible2)
        mySprite4 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible3)
        tiles.placeOnRandomTile(mySprite, sprites.dungeon.collectibleInsignia)
        tiles.placeOnRandomTile(mySprite2, sprites.dungeon.chestOpen)
        tiles.placeOnRandomTile(mySprite3, sprites.dungeon.chestClosed)
        tiles.placeOnRandomTile(mySprite4, sprites.castle.rock1)
        tileUtil.replaceAllTiles(sprites.dungeon.collectibleInsignia, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.dungeon.chestOpen, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.dungeon.chestClosed, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.castle.rock1, assets.tile`transparency16`)
    }
})
game.onUpdateInterval(100, function () {
    if (score == 29) {
        count = 10
        game.stats = true
const rcRender = Render.raycastingRender
Render.moveWithController(2, 2, 1)
        trans16 = image.create(16, 16)
        scene.setBackgroundImage(img`
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            ................................................................................................................................................................
            
            `)
        let map = tiles.createTilemap(hex``, img`
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . 2 2 2 . . 2 2 2 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . 2 . . . . . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 . . . . . . . 2 . . 2 
2 . . . 2 2 2 2 . . 2 2 2 . . 2 
2 . . . . . . . . . . . . . . 2 
2 . . . . . . . . . . . . . . 2 
2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 
`, [trans16, sprites.castle.tileGrass2, sprites.builtin.forestTiles0], TileScale.Sixteen);
// tiles.setCurrentTilemap(map)
        tm = tilemap`level3`
        // tm.scale =2// TileScale.Eight
        tiles.setCurrentTilemap(tm)
        const tilemapScale = 1 << game.currentScene().tileMap.scale
rcRender.sprSelf.setPosition(8 * tilemapScale, 8 * tilemapScale)
        createSprite(8, 7, 6, 10, texturesDuck, SpriteKind.Enemy)
        createSprite(6, 7, 6, 10, texturesDonut, SpriteKind.Enemy)
        createSprite(5, 8, 6, 10, texturesDog, SpriteKind.Enemy)
        sprPriness2 = createSprite(11, 8, 6, 10, texturesPrincess2, SpriteKind.Enemy)
        sprHero = createSprite(10, 8, 6, 10, texturesHero, SpriteKind.Enemy)
        sprSkelly = createSprite(9, 9, 6, 10, texturesSkelly, SpriteKind.Enemy)
        sprPriness = createSprite(10, 7, 6, 10, texturesPrincess, SpriteKind.Enemy)
        sprPlane = createSprite(9, 7, 6, 10, texturesPlane, SpriteKind.Enemy)
        cake = createSprite(2, 2, 4, 2, texturesBigCake, SpriteKind.Enemy)
        fish = createSprite(7, 9, 0, 0, texturesFish, SpriteKind.Enemy)
        controller.B.repeatDelay = 0
rcRender.wallZScale = 2
zOffset = 3
        rcRender.setZOffset(rcRender.sprSelf, zOffset, 0)
let fov = Render.defaultFov
score = 30
        info.startCountdown(120)
        mySprite = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible)
        mySprite2 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible1)
        mySprite3 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible2)
        mySprite4 = sprites.create(img`
            . . b b b b . . 
            . b 5 5 5 5 b . 
            b 5 d 3 3 d 5 b 
            b 5 3 5 5 1 5 b 
            c 5 3 5 5 1 d c 
            c d d 1 1 d d c 
            . f d d d d f . 
            . . f f f f . . 
            `, SpriteKind.collectible3)
        tiles.placeOnRandomTile(mySprite, sprites.dungeon.collectibleInsignia)
        tiles.placeOnRandomTile(mySprite2, sprites.dungeon.chestOpen)
        tiles.placeOnRandomTile(mySprite3, sprites.dungeon.chestClosed)
        tiles.placeOnRandomTile(mySprite4, sprites.castle.rock1)
        tileUtil.replaceAllTiles(sprites.dungeon.collectibleInsignia, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.dungeon.chestOpen, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.dungeon.chestClosed, assets.tile`transparency16`)
        tileUtil.replaceAllTiles(sprites.castle.rock1, assets.tile`transparency16`)
    }
})
game.onUpdateInterval(100, function () {
    if (score == 44) {
        game.over(true)
    }
})
